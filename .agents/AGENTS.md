...
2. si la demande est explicitement une **publication APK seule**, appliquer immédiatement le **fast path APK prioritaire** du `AGENTS.md` racine : cette demande autorise l'agent à déclencher et suivre lui-même la release jusqu'au résultat. Quand la conversation ne dispose que du **connecteur GitHub**, le chemin natif prioritaire de `SEENIT-RELEASE-005` est la commande exacte `/release-apk` — ou `/release-apk android12_smoke=true` lorsqu'un TNR Android 12 est requis — publiée sur l'issue de contrôle #102 ; le workflow `SeenIt Release Control` vérifie l'auteur, `main`, la version, l'immuabilité et l'anti-doublon avant de déclencher `Validate & Release SeenIt`. Ce chemin ne dépend ni de `gh`, ni d'un token shell, ni d'un navigateur authentifié. Lire aussi `docs/specifications/release-control.md`. Les anciens outils directs/CLI/navigateur restent des secours historiques, pas un prérequis ;
