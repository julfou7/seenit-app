...
2. si la demande est explicitement une **publication APK seule**, appliquer immédiatement le **fast path APK prioritaire** du `AGENTS.md` racine : `release:status` devient le préflight canonique et les étapes 3 à 5 ci-dessous ne sont reprises que si ce statut révèle un blocage sortant du fast path ;
