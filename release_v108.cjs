...
async function start() {
  ...
  async function checkStatus() {
    ...
    if (run.status === "completed") {
      if (run.conclusion === "success") {
        console.log("GitHub Action finished with success! Deploying release v1.0.8...");
  ...
  async function deployRelease(rId) {
    try {
      ...
      execSync("unzip -o app.zip -d app-release-v108");
      
      const release = await apiReq(`/repos/${repo}/releases`, "POST", {
        tag_name: "v1.0.8", name: "v1.0.8", body: "Fix TMDB API Key fallback on native APK"
      });
      console.log("Created Release ID:", release.id);
      
      execSync(`curl -X POST -H "Authorization: token ${token}" -H "Content-Type: application/vnd.android.package-archive" --data-binary @app-release-v108/app-debug.apk "https://uploads.github.com/repos/${repo}/releases/${release.id}/assets?name=SeenIt-v1.0.8.apk"`);
      console.log("Release v1.0.8 completed successfully!");
    } catch(e) {
