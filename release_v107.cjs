...
async function start() {
  ...
  async function checkStatus() {
    ...
    if (run.status === "completed") {
      if (run.conclusion === "success") {
        console.log("GitHub Action finished with success! Deploying release v1.0.7...");
  ...
  async function deployRelease(rId) {
    try {
      ...
      execSync("unzip -o app.zip -d app-release-v107");
      
      const release = await apiReq(`/repos/${repo}/releases`, "POST", {
        tag_name: "v1.0.7", name: "v1.0.7", body: "Fix auto-updater proxy and versions"
      });
      console.log("Created Release ID:", release.id);
      
      execSync(`curl -X POST -H "Authorization: token ${token}" -H "Content-Type: application/vnd.android.package-archive" --data-binary @app-release-v107/app-debug.apk "https://uploads.github.com/repos/${repo}/releases/${release.id}/assets?name=SeenIt-v1.0.7.apk"`);
      console.log("Release v1.0.7 completed successfully!");
    } catch(e) {
