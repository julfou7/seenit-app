...
7. Une demande explicite de publication autorise l'agent à déclencher lui-même `Validate & Release SeenIt` sur `main` avec `release_apk=true` et à mener la release jusqu'au résultat. L'ordre obligatoire des voies est : outil GitHub direct de `workflow_dispatch` ; fallback borné `npm run release:dispatch` (qui encapsule `gh workflow run build-apk.yml`) ; puis interface GitHub Actions via un navigateur authentifié contrôlable par l'agent. Dans le navigateur, sélectionner exactement `main`, `release_apk=true` et `android12_smoke=false` sauf demande ou risque natif explicite. L'absence de `gh` ou de token shell ne constitue pas un blocage tant que cette troisième voie est disponible. Ne demandez jamais à l'utilisateur d'effectuer le clic lui-même avant d'avoir réellement épuisé les trois voies, et ne cherchez **ni sur le Web ni via des plugins** un autre mécanisme.
8. Avant tout dispatch, vérifier qu'aucun run de release portant le même SHA/version n'est déjà actif. Suivre ensuite uniquement le run déclenché jusqu'à l'APK signé, son `.sha256` et la release immuable. Diagnostiquer les échecs dans le même chantier et reporter la mesure « demande → workflow » fournie par `release:dispatch` ou calculée au moment du dispatch direct/navigateur.
