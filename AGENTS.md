...
## 0.0t Fast path prioritaire — correctif APK à valider sur téléphone

Quand l'utilisateur demande un **correctif APK qui doit être essayé sur son téléphone**, le mandat couvre la correction **et sa mise à disposition dans la mise à jour intégrée SeenIt**. Il ne doit pas avoir à relancer séparément « publie APK » une fois le correctif mergé.

Objectif : **correctif prêt/mergé → mise à jour installable dans SeenIt en ≤ 10 minutes**, hors temps de diagnostic du bug lui-même.

1. Corriger selon les sections 0.1 et 0.2, obtenir les tests ciblés et la validation canonique, puis merger le correctif sur `main`.
2. Si la prochaine version n'est pas encore préparée, enchaîner `/prepare-release-apk` sur #102, attendre la validation explicite du SHA candidat, merger l'unique candidate compatible puis reprendre immédiatement. Ne pas demander une nouvelle confirmation utilisateur.
3. Une fois `main` sur le prochain patch et sa validation `push` du **SHA exact** verte, publier exactement `/release-terrain` sur #102. Le contrôleur refuse ce mode sans preuve `main` verte ; le fast path ne transforme jamais l'absence de validation en gain de temps.
4. `/release-terrain` publie une **release GitHub normale et officielle**, visible par `releases/latest`. Il n'existe ni canal candidate visible par l'utilisateur, ni APK manuelle, ni seconde compilation « stable » après le terrain. SeenIt notifie la nouvelle version et conserve son parcours actuel téléchargement → vérification → Package Installer.
5. Le chemin terrain réutilise la validation `main` déjà verte au lieu de rejouer SPEC/lint/tests unitaires/audit dans le job de release. Il conserve signature `seenit`, `applicationId`, version/versionCode croissants, garde d'immuabilité, build Web, sync Capacitor, contrat Android après sync, Gradle, APK canonique, SHA-256, release GitHub et notification. Les smokes émulateur Android 36/12 sont **non bloquants et sautés avant ce premier terrain ordinaire**.
6. Utiliser à la place la release complète `/release-apk` lorsque le changement touche précisément la signature, l'identité/package, une migration d'installation N→N+1 ou le mécanisme de téléchargement/installation de mise à jour : dans ces cas, le smoke d'upgrade prouve directement la surface modifiée.
7. **Premier terrain rouge/KO = prétention de correction invalidée.** Avant un nouveau patch, ne pas enchaîner un autre micro-correctif isolé sur la même hypothèse. Reconstituer le chemin de production réel de bout en bout (UI → hook/store → client → backend/native → fournisseur selon la surface), localiser la frontière fautive avec des preuves/instrumentation si nécessaire et ajouter un TNR qui passe par le **point d'entrée de production réel** (façade, singleton, adapter, route ou wiring effectif), en plus du cas utilisateur. Le terrain KO et ce nouveau diagnostic sont enregistrés dans le checkpoint stable du chantier.
8. Une tentative terrain KO produit ensuite le patch suivant dans **le même chantier** et reprend automatiquement cette boucle. Une tentative terrain OK termine le chantier sans promotion, recompilation ou release supplémentaire.
9. Mesurer sur chaque boucle utile : nombre de versions terrain nécessaires, temps merge/« correctif prêt » → release installable/notifiée, nombre de handoffs avant fin et toute P1 apparue dans les 24 h suivant une release. Ces métriques servent à réduire les itérations, pas à ajouter un portail administratif.

Ce fast path est le chemin par défaut des correctifs APK destinés au téléphone du propriétaire. La release complète reste un outil de preuve ciblé pour les changements qui affectent eux-mêmes l'installation ou les invariants APK.
...
4. Pendant le développement, exécuter les tests ciblés jusqu'à ce qu'ils soient verts. Exécuter ensuite une seule fois la validation complète applicable avant le premier push. **Sauf fallback sans egress strictement borné de la section 0.3**, GitHub Actions confirme un état local vert ; elle ne sert pas de debugger pour une erreur reproductible localement.
...
7. Après merge d'un correctif qui doit être publié, basculer directement vers le fast path terrain 0.0t lorsqu'une validation sur téléphone fait partie de la demande, vers le fast path release-only 0.0 pour une publication complète explicite, ou vers le fast path PWA/runtime 0.0a selon la surface, sans relire une seconde fois tout le contexte produit.
...
2. Avant toute acquisition locale, rechercher un workspace SeenIt déjà présent dans **les surfaces réellement disponibles du runtime** (racines de travail usuelles, workspace/devcontainer/Codespace connu, branche de chantier déjà matérialisée). S'il existe, vérifier son état, confronter son HEAD/sa branche au `main` ou à la branche du chantier, puis le réutiliser. **Un nouveau prompt, une reprise ou une nouvelle phase du même chantier n'est jamais une raison de recloner.** Vérifier seulement un chemin attendu (par exemple `/mnt/data/seenit-app`) ne prouve pas l'absence de workspace.
   Avant **toute commande de clone ou autre acquisition locale**, établir puis publier dans l'issue concernée un reçu pré-acquisition portant le marqueur exact `<!-- seenit-workspace-acquisition -->` et les quatre lignes suivantes :
   ```
...
acquisition minimale : <mode retenu et borne> | sans objet
   ```
   Si le résultat est `trouvé et réutilisé`, ou si API-first suffit encore au diagnostic/modification, **l'acquisition est interdite**. Un runtime prétendument « neuf » ne peut être invoqué que si son état réel a été établi ; une reprise conversationnelle ne constitue pas cette preuve.
3. Une nouvelle acquisition n'est autorisée que si le reçu conclut `absent/inexploitable` **et** identifie une exécution locale réellement nécessaire que le connecteur/API, une branche existante, un Codespace ou le fallback distant ne permettent pas d'accomplir. Acquérir alors le dépôt **une seule fois par environnement** depuis le SHA/branche canonique, avec le mode minimal compatible (`--depth`, clone partiel ou checkout partiel) plutôt qu'un historique complet par défaut. Ne jamais utiliser `git clone` comme simple sonde d'egress.
...
5. **Fallback sans egress strictement borné.** Si le runtime bloque réellement l'accès Git/npm **et** qu'aucun workspace/cache exact ne permet d'exécuter `validate:change`, appliquer `docs/process/agent-remote-workspace.md`. Le connecteur/API écrit uniquement dans `agent-staging/**` ; `Agent Remote Validate` checkout le **SHA exact**, calcule son merge-base avec le `main` courant, construit le devcontainer et exécute le contrat canonique. Un SHA rouge reste en quarantaine. Un SHA vert peut seulement être promu en faisant pointer la branche de chantier vers **ce même SHA, sans réécriture** ; la quarantaine n'est jamais mergée directement. C'est l'unique exception au garde local avant push et à l'interdiction d'utiliser la CI comme boucle de correction distante. Elle est interdite dès qu'un chemin local sain est disponible.
6. Lors d'une reprise, repartir d'abord de l'issue, de la PR ou de la branche existante et du dernier jalon contenant la **prochaine action exacte**. **Si le prompt de reprise est générique et ne nomme aucune issue, PR ou branche** (par exemple « reprends le chantier », « où en est le dernier chantier ? », « continue le dernier travail »), commencer obligatoirement par **les PR ouvertes du dépôt**, triées par activité récente, avant toute recherche d'issues fermées, d'anciennes branches ou tout recours à un checkpoint conversationnel. Une PR ouverte est le premier candidat de reprise car elle fournit directement la branche, le SHA, l'issue liée, les checks et la prochaine action ; lire sa description, l'issue qu'elle ferme/référence, ses commentaires et son état CI. Si plusieurs PR ouvertes sont plausibles, retenir celle qui correspond au dernier travail utilisateur d'après leur activité et leur contexte explicite, sans inventer de priorité ; si aucune PR ouverte n'est exploitable, rechercher ensuite les issues ouvertes avec checkpoint `<!-- seenit-resume -->`, puis les branches actives, et seulement ensuite l'historique fermé. **Un vieux checkpoint conversationnel, une issue terminée ou une branche résiduelle ne doit jamais supplanter une PR ouverte plus récente qui matérialise un chantier actif.** Ne pas reconstruire l'historique complet si ce jalon et les index canoniques bornent déjà le travail restant. **Reprise identifiée : pas de recherche globale.** Si l'issue, la PR ou la branche du chantier et un checkpoint exploitable sont déjà connus, vérifier `main`, lire ces références et exécuter la prochaine action exacte sans relancer la recherche générale des issues ouvertes/fermées, PR, commits ou audits. Relancer cette recherche seulement si le périmètre change, si le checkpoint est absent ou ambigu, si `main` révèle une contradiction pertinente ou si une nouvelle anomalie hors périmètre apparaît.
7. **Avant chaque fin de session SeenIt**, quelle qu'en soit la cause (chantier terminé, blocage réel, limite de session, handoff volontaire ou involontaire), conserver **un seul checkpoint de reprise stable** dans l'issue fonctionnelle concernée, identifié par le marqueur exact `<!-- seenit-resume -->`. Le créer s'il n'existe pas puis le **mettre à jour en place** au lieu d'empiler de nouveaux commentaires de reprise. Ce checkpoint contient au minimum : état exact du chantier, SHA/branche/PR ou `main` courant, fichiers ou surfaces modifiés, tests déjà verts, validations rouges avec les numéros de runs/jobs utiles, dernier résultat terrain s'il existe, blocage éventuel, prochaine action exacte et critère de fin. Les commentaires de bail `<!-- seenit-agent-lease -->`, preuves de CI/release et échanges de diagnostic restent distincts ; le checkpoint `seenit-resume` est la synthèse unique que la reprise lit en premier. Si le chantier est terminé, écrire explicitement **`Prochaine action : aucune — chantier terminé`** avec les preuves de merge/release/déploiement. La réponse finale à l'utilisateur résume ce même checkpoint ; ne jamais terminer une session SeenIt sur un statut vague ou uniquement sur un message d'erreur.
8. **Avant tout rendu de main ChatGPT interactif**, si la surface Automations est disponible, vérifier que `SeenIt — reprise autonome` reste active. Si le chantier est inachevé, son checkpoint doit être réellement reprenable (`HANDOFF_READY`, ou `WAITING` seulement avec un événement externe précis) et ne doit pas être masqué par un bail concurrent. Ne jamais désactiver cette automatisation pour signifier « rien à faire ».
9. **Avant de déclarer un blocage GitHub ou un manque d'accès**, distinguer les permissions du token de la surface exposée par le connecteur. Un refus d'un endpoint générique ou une allowlist du connecteur n'est pas une preuve d'insuffisance des droits. Rechercher/inventorier puis tenter les actions GitHub dédiées adaptées à la ressource (par exemple jobs, logs, checks, artifacts, workflows, issues/PR) avant de conclure qu'une information ou une action est inaccessible. Un blocage utilisateur ne peut être déclaré qu'après échec ou absence prouvée de l'action dédiée **et** épuisement du chemin connector-only documenté dans le dépôt.
10. Laisser les opérations distantes longues — CI, release et déploiement — à GitHub Actions. Ne pas consommer une fenêtre d'exécution en polling rapproché, sauf demande explicite de suivi synchrone.
...
**Hors fast paths des sections 0.0, 0.0t, 0.0a et 0.1 :**
...
4. Pour un nouveau chantier, ou lorsqu'aucune issue/PR/branche n'est déjà identifiée, rechercher systématiquement les issues GitHub **ouvertes et fermées liées au sujet**, ainsi que les PR, commits, audits et documents pertinents, afin de reprendre l'historique existant. Réutiliser ou rouvrir l'issue pertinente lorsqu'elle existe et éviter les doublons. Pour une reprise identifiée, appliquer la section 0.3 et ne pas rejouer cette recherche globale sauf si le périmètre change, si le checkpoint est absent ou ambigu, si `main` révèle une contradiction pertinente ou si une nouvelle anomalie hors périmètre apparaît.
   - Lorsque l'utilisateur demande simplement de « traiter », « prendre la next issue », « prochaine issue » ou équivalent sans fixer d'autre priorité, sélectionner **la plus ancienne issue ouverte actionnable par date de création**. Ne sauter une issue que pour une exclusion utilisateur explicite, un statut `[WAITING]`/`[BLOCKED]`/`Terrain` sans événement de déblocage, une issue de contrôle non fonctionnelle, ou un bail concurrent actif ; dans ce cas poursuivre avec la suivante par ancienneté. Ne jamais substituer silencieusement un tri par priorité, numéro, activité récente ou préférence de l'agent.
...
Pendant la mise au point, exécuter les tests ciblés utiles. **Hors fallback sans egress strictement borné de la section 0.3, avant le premier push d'un arbre Git modifié, `npm run validate:change` doit être vert sur cet arbre exact.** Cette commande est le garde local canonique : politique et syntaxe des workflows, SPEC, matérialisation Android, classification, contrat de changement, TypeScript, tests unitaires, contrat Android conditionnel, audit de dépendances applicable et build. Un agent ne pousse pas un changement dont cette validation complète est rouge ; GitHub Actions confirme le même contrat et ne sert pas de boucle de correction distante. Le seul fallback autorisé matérialise d'abord le changement dans `agent-staging/**`, exige `Agent Remote Validate` vert sur le SHA exact et interdit toute promotion de ce SHA tant que le contrôle est rouge.
...
Pour une release complète, le job Android cible (API courante) reste bloquant. Pour une **release terrain** de correctif déjà validé, `fast_terrain=true` réutilise la validation verte du SHA `main` exact et laisse le téléphone réel jouer son rôle de validation prioritaire : les smokes Android 36/12 ne retardent pas la première mise à disposition. Les invariants d'installation, signature, identité, version, build, digest et publication restent obligatoires. Le chemin complet redevient obligatoire si le changement affecte directement l'installation/mise à jour ou ces invariants.
...
Une modification Android intentionnelle doit être couverte par un test et, si elle touche un invariant, par la SPEC/contrat Android. Lors d'une release APK, matérialiser d'abord la clé release, exécuter `npm run test:android`, puis `npx cap sync android`, puis de nouveau `npm run test:android` avant Gradle. Pour le fast terrain, la première exécution du contrat Android provient de la validation canonique verte du SHA `main`; la revalidation après `cap sync` reste exécutée dans le job de release.
...
- Ajouter dans cette section un sous-bloc **Difficultés rencontrées / amélioration continue** : y consigner uniquement les blocages, détours, limites d'outillage, échecs de CI/TNR ou hypothèses invalidées réellement rencontrés pendant le chantier, avec l'enseignement concret ou l'amélioration durable associée. Ne jamais inventer de difficulté ; écrire « Aucune difficulté notable » lorsqu'il n'y en a pas. Lorsqu'une difficulté a déclenché la règle proactive de la section 7, citer l'issue réutilisée, rouverte ou créée.

### 📌 Impact & Mode de déploiement
- Classe `light`, `backend` ou `apk`, et préciser si l'APK attend la prochaine release groupée.

### 💾 Checkpoint de fin
- Résumer le checkpoint persistant `<!-- seenit-resume -->` enregistré et mis à jour en place dans l'issue : état exact, branche/PR/SHA, validations utiles, dernier terrain connu, blocage éventuel, prochaine action exacte et critère de fin.
