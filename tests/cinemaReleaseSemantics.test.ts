...
test('SEENIT-DISCOVER-001 donne la priorité au payload release_dates sur une ancienne preuve en cache', () => {
  ...
  assert.equal(hasFrenchTheatricalCinemaEvidence({ id, media_type: 'movie' }), true, 'la preuve fraîche est réutilisable entre cartes');
  assert.equal(hasFrenchTheatricalCinemaEvidence(movieWithFrenchRelease(4, -5, id)), false, 'un détail digital invalide la preuve positive précédente');
  assert.equal(hasFrenchTheatricalCinemaEvidence({ id, media_type: 'movie' }), false, 'le cache positif doit être purgé après invalidation');
});
