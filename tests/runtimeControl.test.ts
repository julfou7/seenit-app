...
test('AGENTS interdit de rendre la main pour une simple absence de workflow_dispatch direct', () => {
  assert.match(agents, /Fast path prioritaire — publication PWA \/ runtime canonique/);
  assert.match(agents, /publier exactement `\/deploy-pwa` sur l'issue de contrôle #57/);
  assert.match(agents, /n'est pas un blocage utilisateur/);
  assert.match(agents, /ne jamais demander à l'utilisateur de cliquer sur « Run workflow »/);
  assert.match(agents, /Un retrait sans remplacement constitue une régression de processus/);
});