...
test('SEENIT-PERF-001 réserve les skeletons au chargement réellement froid', () => { assert.match(detailSource, /peekRenderableMediaDetails/); assert.match(detailSource, /peekUniverseAndCollection/); assert.match(detailSource, /setCollectionLoading\(!cachedRelations\)/); assert.match(detailSource, /loading="eager" decoding="async"[\s\S]{0,120}fetchPriority="high"/); });
