...
async function checkStatus() {
  ...
  if (run.status === "completed") {
    if (run.conclusion === "success") {
      console.log("GitHub Action finished with success! Deploying release...");
      await deployRelease();
    } else {
...
async function deployRelease() {
  try {
    ...
    execSync("unzip -o app.zip -d app-release-v103");
    
    const release = await apiReq(`/repos/${repo}/releases`, "POST", {
      tag_name: "v1.0.3", name: "v1.0.3", body: "Fix Android status bar overlay issue with pt-safe padding"
    });
    console.log("Created Release ID:", release.id);
    
    execSync(`curl -X POST -H "Authorization: token ${token}" -H "Content-Type: application/vnd.android.package-archive" --data-binary @app-release-v103/app-debug.apk "https://uploads.github.com/repos/${repo}/releases/${release.id}/assets?name=SeenIt-v1.0.3.apk"`);
