...
async function checkStatus() {
  ...
  if (run.status === "completed") {
    if (run.conclusion === "success") {
      console.log("GitHub Action finished with success! Deploying release v1.0.4...");
      await deployRelease();
    } else {
...
async function deployRelease() {
  try {
    ...
    execSync("unzip -o app.zip -d app-release-v104");
    
    const release = await apiReq(`/repos/${repo}/releases`, "POST", {
      tag_name: "v1.0.4", name: "v1.0.4", body: "Fix Google 2FA 400 error by adding full auth domains and robust IndexedDB persistence"
    });
    console.log("Created Release ID:", release.id);
    
    execSync(`curl -X POST -H "Authorization: token ${token}" -H "Content-Type: application/vnd.android.package-archive" --data-binary @app-release-v104/app-debug.apk "https://uploads.github.com/repos/${repo}/releases/${release.id}/assets?name=SeenIt-v1.0.4.apk"`);
    console.log("Release v1.0.4 completed successfully!");
  } catch(e) {
