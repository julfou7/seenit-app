...
- Une synchronisation externe ne déduit jamais une suppression depuis une simple absence. Une source
  ...
  Ce signal vaut dé-vu en full comme en delta, sous réserve de la propriété Plex définie par `SEENIT-PLEX-006` ;
  un 404, un timeout ou une simple absence du snapshot ne valent jamais confirmation non-vue.
...
- **SEENIT-PLEX-006** — Une synchronisation Plex, **full ou delta**, réconcilie l’état vu et le dé-vu
  **sans donner à Plex l’autorité sur les progressions créées hors Plex**. Un `viewCount > 0` peut ajouter
  ...
  à jour du miroir `plexWatchState` seule est silencieuse : elle n’est ni comptée ni notifiée comme un dé-vu.
  Les clients antérieurs au protocole de provenance sûre ne reçoivent pas d’états `watched=false`.
