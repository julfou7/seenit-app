...
- **SEENIT-APK-005** — Le lancement Android n'affiche qu'un seul branding de démarrage : le splash
  ...
  status bar overlay la WebView avec un fond transparent et des icônes claires. Sur Android 15/16,
...
- Explorer résout cette preuve via les payloads TMDB minimaux `release_dates` ou `content_ratings`.
  Une page `Tout` complète est coalescée dans un transport backend streamé. Avec le tri **Populaires**,
  les classifications sûres sont rendues progressivement sur la première page comme sur les suivantes :
  dès qu'une carte sûre apparaît, sa position relative est figée pour la génération courante et les
  nouvelles cartes sûres s'ajoutent sans déplacer celles déjà visibles ; le résultat complet conserve
  ce même ordre de présentation stable. Avec un tri explicite par note, date ou titre, la page attend sa
  résolution parentale complète avant d'afficher ses nouveaux résultats afin de respecter exactement le
  tri choisi sans réordonner une grille en cours de chargement. La concurrence fournisseur reste bornée :
  24 appels constituent la fenêtre nominale et, si cette fenêtre reste bloquée, seule la queue de la page
  courante peut être admise après 1 s, sans jamais dépasser 40 appels pour cette page. Une fiche média
  complète ne doit jamais être hydratée uniquement pour appliquer le filtre d'âge.
...
- Pendant un filtre d'âge actif, Explorer maintient une **fenêtre bornée de deux pages Discover source
  ...
  pagination normale, l'ordre des pages reste inchangé et un changement de filtre invalide
...
- **SEENIT-COMMUNITY-001** — Les réactions Reddit d'un épisode restent verrouillées tant que cet épisode
  ...
  requête naturelle courte de la forme `English title S3 E1`. Le titre d'affichage SeenIt reste localisé
  en `fr-FR`, mais il n'est plus assimilé au titre de recherche communautaire.
- Au moment exact où l'utilisateur ouvre Reddit, SeenIt demande à TMDB le titre `en-US` du média identifié
  par son **TMDB ID exact** et l'utilise lorsqu'il est disponible. Ce choix vise le libellé majoritaire du
  corpus Reddit pour les séries internationales sans modifier les métadonnées persistées. Le terrain
  `Berlin et La Dame à l'hermine` a montré que le seul titre FR pouvait ramener surtout des discussions
  générales alors que Reddit emploie largement `Berlin and the Lady with an Ermine`.
- Si la résolution anglaise échoue ou retourne un titre vide, SeenIt garde le titre visible comme premier
  fallback puis le titre original. Saison et épisode restent deux termes séparés utilisant leurs numéros
  humains **sans padding**. Aucun `AND` / `OR`, `title:`, guillemet/parenthèse de filtrage, nom
  d'épisode ou consigne IA n'est ajouté.
