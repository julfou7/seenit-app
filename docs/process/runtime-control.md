...
2. Si l'outil direct `workflow_dispatch` n'est pas disponible, **ne pas déclarer de blocage utilisateur** : publier exactement `/deploy-pwa` sur l'issue #57 avec le connecteur GitHub.
...
L'absence d'un bouton, d'une action connecteur ou d'un binaire `gh` local n'est donc **pas** un blocage pour une publication PWA : le contrôleur GitHub natif fait partie du contrat SeenIt.
