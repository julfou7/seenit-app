...
3. Fixer explicitement l'ancienne révision à 100 %, exporter le service, retirer uniquement les métadonnées source incompatibles, injecter le digest candidat, forcer `NODE_ENV=production` au runtime et valider la configuration avec `services replace --dry-run`.
