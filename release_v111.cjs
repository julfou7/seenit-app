...
async function start() {
  ...
  async function checkStatus() {
    ...
    if (run.status === "completed") {
      if (run.conclusion === "success") {
        console.log("GitHub Action finished with success! Deploying release v1.1.1...");
  ...
  async function deployRelease(rId) {
    try {
      ...
      const existingRel = Array.isArray(rels) ? rels.find(r => r.tag_name === "v1.1.1") : null;
      if (existingRel) {
        console.log("Deleting previous release ID:", existingRel.id);
        await apiReq(`/repos/${repo}/releases/${existingRel.id}`, "DELETE");
        try { await apiReq(`/repos/${repo}/git/refs/tags/v1.1.1`, "DELETE"); } catch(e){}
      }
      ...
      execSync("unzip -o app.zip -d app-release-v111");
      
      const release = await apiReq(`/repos/${repo}/releases`, "POST", {
        tag_name: "v1.1.1", name: "v1.1.1", body: "Correctif de la recherche de mises à jour : requête directe de l'API GitHub pour autoriser la vérification sur l'APK autonome sans restriction CORS/authentification backend."
      });
      console.log("Created Release ID:", release.id);
      
      execSync(`curl -X POST -H "Authorization: token ${token}" -H "Content-Type: application/vnd.android.package-archive" --data-binary @app-release-v111/app-debug.apk "https://uploads.github.com/repos/${repo}/releases/${release.id}/assets?name=SeenIt-v1.1.1.apk"`);
      console.log("Release v1.1.1 completed successfully!");
    } catch(e) {
