  ...
  return (
    <div ref={mainScrollRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}
              ...
              {(() => {
                ...
                return <div className="flex flex-col gap-2.5 mt-2">
                  {!tmdbDetails && !year ? <div className="h-4 w-36 bg-zinc-800/80 rounded animate-pulse my-0.5" /> : <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] sm:text-xs text-zinc-300 font-medium">
                    {(year || (!isSeries && isAtCinema)) && <span className="flex items-center gap-1.5">{year && !(!isSeries && isAtCinema) && <span>{year}</span>}{!isSeries && (isAtCinema ? <span className="text-[#E5A93D] font-extrabold inline-flex items-center gap-1 bg-[#E5A93D]/10 px-2 py-0.5 rounded border border-[#E5A93D]/30 text-[10px]"><Ticket size={11} className="text-[#E5A93D]" />Au cinéma</span> : isUpcoming ? <span className="text-purple-400 font-extrabold inline-flex items-center gap-1 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/30 text-[10px]"><Calendar size={11} className="text-purple-400" />À venir</span> : null)}</span>}
      ...
      <div ref={tabsRef} className="px-4 mt-6 sticky top-0 z-20 bg-black/90 backdrop-blur-xl pt-2 pb-2"><div className="flex gap-1 bg-zinc-900 p-1 rounded-full border border-white/5">
        ...
        {((tmdbDetails?.aggregate_credits?.cast || tmdbDetails?.credits?.cast)?.length > 0) && <button onClick={() => { handleTabChange('casting'); setShowAllCast(false); }} className={cn("flex-1 py-2 text-xs font-bold tracking-wider uppercase transition-all rounded-full touch-manipulation", activeTab === 'casting' ? "bg-zinc-800 text-[#E5A93D] shadow-lg" : "text-zinc-500")}>Casting</button>}
      </div></div>
      ...
      <div className="p-4 min-h-[400px] pb-6">
        <div id="section-about" className="scroll-mt-40 space-y-6 animate-in fade-in duration-200">
          ...
          {(collectionData && collectionData.parts && collectionData.parts.length > 0) && <div><h3 className="text-xs font-bold uppercase text-zinc-500 tracking-wider mb-3">Ordre de visionnage</h3><div className="flex overflow-x-auto gap-3.5 hide-scrollbar py-2 px-1 -mx-1">{collectionData.parts.map((part: any, idx: number) => <TimelineMediaCard key={`col_part_${part.media_type || 'media'}_${part.id}_${idx}`} media={part} isActive={mediaKeyFrom(part) === requestedMediaKey} onClick={() => onShowClick && onShowClick(part.id, part.media_type || (part.title ? 'movie' : 'tv'))} />)}</div></div>}
          {(universeData && universeData.parts && universeData.parts.length > 0) && <div className={collectionData?.parts?.length > 0 ? "mt-6" : ""}><h3 className="text-xs font-bold uppercase text-zinc-500 tracking-wider mb-3">{universeData.parts[0]?.seenitRelationKind === 'franchise' ? 'Dans la même franchise' : 'Dans le même univers'}</h3><div className="flex overflow-x-auto gap-3.5 hide-scrollbar py-2 px-1 -mx-1">{universeData.parts.map((part: any, idx: number) => <TimelineMediaCard key={`univ_part_${part.media_type || 'media'}_${part.id}_${idx}`} media={part} isActive={mediaKeyFrom(part) === requestedMediaKey} onClick={() => onShowClick && onShowClick(part.id, part.media_type || (part.title ? 'movie' : 'tv'))} />)}</div></div>}
          ...
          <div><h3 className="text-xs font-bold uppercase text-zinc-500 tracking-wider mb-2">Où regarder</h3>{(() => {
            ...
            if (hasProviders) return <div className="flex items-center gap-2 flex-wrap">{sortedProviders.map((provider: any, idx: number) => {
      ...
      {isDownloadModalOpen && <DownloadModal isOpen={isDownloadModalOpen} onClose={() => { setIsDownloadModalOpen(false); setDownloadTargetSeason(undefined); setDownloadTargetEpisode(undefined); }} title={title} originalTitle={tmdbDetails?.original_title || tmdbDetails?.original_name || (show as any)?.originalTitle} year={(tmdbDetails?.release_date || tmdbDetails?.first_air_date || show?.firstAirDate)?.slice(0, 4)} mediaType={isSeries ? 'tv' : 'movie'} tmdbId={effectiveTmdbId} tvdbId={tmdbDetails?.external_ids?.tvdb_id || (show as any)?.tvdbId} imdbId={tmdbDetails?.external_ids?.imdb_id || (show as any)?.imdbId} posterPath={posterPath || tmdbDetails?.poster_path || show?.posterPath} initialSeason={downloadTargetSeason} initialEpisode={downloadTargetEpisode} totalSeasons={tmdbDetails?.number_of_seasons || tmdbDetails?.seasons?.filter((s: any) => s.season_number > 0)?.length || 1} seasonsData={tmdbDetails?.seasons} onSuccessToast={(msg) => showToast(msg, 'success', show || undefined)} />}
    </div>
  );
}
